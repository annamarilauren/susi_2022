...
def run_susi(forc, wpara, cpara, org_para, spara, outpara, photopara, start_yr, end_yr, wlocation=None, mottifile=None, peat=None, 
    ...
    stand = Stand(nscens, yrs, spara['canopylayers'], spara['n'], ageSim, mottifile, photopara)   
    stand.update()
    ...
    for r, dr in enumerate(zip(spara['ditch depth west'], spara['ditch depth 20y west'], \
        ...
        # **************  Biogeochemistry ***********************************
            ...
            nonwoodylitter = (stand.nonwoodylitter + groundvegetation.nonwoodylitter)/10000.     #conversion kg/ha/yr -> kg/m2/yr
            woodylitter = stand.woodylitter/10000. 
            ...
            n_nonwoodylitter = (stand.n_nonwoodylitter + groundvegetation.n_litter)/10000. 
            n_woodylitter = stand.n_woodylitter/10000. 
            esN.run_yr(forc.loc[str(yr)], df_peat_temperatures, dfwt, n_nonwoodylitter, n_woodylitter)
            out.write_esom(r, year+1, 'N', esN)
            
            p_nonwoodylitter = (stand.p_nonwoodylitter + groundvegetation.p_litter)/10000.
            p_woodylitter = stand.p_woodylitter/10000. 
            esP.run_yr(forc.loc[str(yr)], df_peat_temperatures, dfwt, p_nonwoodylitter, p_woodylitter)
            out.write_esom(r, year+1, 'P', esP)

            k_nonwoodylitter = (stand.k_nonwoodylitter + groundvegetation.k_litter)/10000. 
            k_woodylitter = stand.k_woodylitter/10000. 
            esK.run_yr(forc.loc[str(yr)], df_peat_temperatures, dfwt, k_nonwoodylitter, k_woodylitter)
            out.write_esom(r, year+1, 'K', esK)

            
            stand.update_nutrient_status(groundvegetation, esN.out  + spara['depoN'] + ferti.release['N'] ,\
                                         esP.out + spara['depoP']+ ferti.release['P'] ,\
                                         esK.out + spara['depoK']+ ferti.release['K'] )
